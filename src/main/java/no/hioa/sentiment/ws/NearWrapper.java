...
public abstract class NearWrapper extends ResponseTransformerRoute
{
	private static final Logger	logger	= LoggerFactory.getLogger("stdoutLogger");

	protected NearWrapper(String path)
	{
	...
	{
		NearRequest incomingRequest = null;
	...
	public abstract NearResponse handle(NearRequest request) throws UnknownHostException;

	private NearRequest translate(String json)
	{
		GsonBuilder gsonBuilder = new GsonBuilder();
		Gson gson = gsonBuilder.create();
		return gson.fromJson(json, NearRequest.class);
	}
}
